...
from src.group_nums import add_group_nums
...
def get_response(input_text,num_return_sequences):
	batch = tokenizer_paraphrase.prepare_seq2seq_batch([input_text],truncation=True,padding='longest',max_length=60, return_tensors="pt").to(torch_device)
	translated = model_paraphrase.generate(**batch,max_length=60,num_beams=10, num_return_sequences=num_return_sequences, temperature=1.5)
...
def paraphrase(ques_adv_lines, num_map):
	...
	for j in range(len(ques_adv_lines)):
		...
		paraphrased_texts = get_response(ques_adv_lines[j], 5)
		if len(num_tokens) !=0:
			candidates = []
	...
	if config.full_cv:
		...
		if is_train:
			...
			for test_batch in test_pairs:
				...
				result = paraphrase(ques_adv_lines, num_map)
				#new_text = []
				...
				for txt in result:
					...
					#new_adv_pair.append((pairs_tested[cur_idx][0], pairs_tested[cur_idx][1], pairs_tested[cur_idx][2], pairs_tested[cur_idx][3], temp_lst))
					new_adv_pair.append((final_adv_text, pairs_tested[cur_idx][1], num_lst, new_idx, temp_lst))
					#breakpoint()
                #print(len(pairs_trained))
					try:
						...
						continue
                #new_adv_pair.append(pairs_tested[1])
                #new_adv_pair.append(pairs_tested[2])
                #new_adv_pair.append(new_idx)
				#breakpoint()
				...
				with open(config.outputs_path + '/outputs_para_roberta.txt', 'a') as f_out:
					f_out.write('Example: ' + str(ex_num) + '\n')
					f_out.write('Source: ' + stack_to_string(sentence_from_indexes(input_lang, test_batch[0])) + '\n')
					f_out.write('Target: ' + stack_to_string(sentence_from_indexes(output_lang, test_batch[2])) + '\n')
					f_out.write('Generated: ' + stack_to_string(sentence_from_indexes(output_lang, test_res)) + '\n')
